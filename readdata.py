...
pt = [-70.3655,47.44883] # Longitude, Latitude of Riviere du Goffre

pt = [-64.3931,48.997777] # Longitude, Latitude of Riviere au Renard
...
pt = [-68.514,48.477] # Longitude, Latitude of Rimouski station

pt = [-68.1337,48.630707] # Longitude, Latitude of Rivière Mitis

pt = [-68.4729,49.05968] # Longitude, Latitude of Rivière aux Outardes

pt = [-67.5314,48.852045] # Longitude, Latitude of Rivière Matane
...
dataa = []
dataa.append([lat_p,lon_p,a_parameter_point,b_parameter_point])
dataa.append([48.44386,-68.56885,a_parameter_point,b_parameter_point])
dataa.append([48.477,-68.514,a_parameter_point,b_parameter_point])
dff = pd.DataFrame(dataa,columns = ["Latitude","Longitude","a","b"])
    
dff_GDF = gpd.GeoDataFrame(dff, geometry=gpd.points_from_xy(dff.Longitude, dff.Latitude))    
...
Bernier_sim = [np.nan,np.nan,np.nan,np.nan,np.nan,3.3146]
...
plt.plot(x,Bernier_sim,'-o',ms=10,label='Bernier_simulation')
plt.xticks(x)
...
Bernier_sim = [np.nan,np.nan,np.nan,np.nan,np.nan,1.1303]
...
Bernier_sim = [np.nan,np.nan,np.nan,np.nan,np.nan,1.4685]
...
Bernier_sim = [np.nan,np.nan,np.nan,np.nan,np.nan,2.0798]
...
#Riviere au Renard
x= [2,5,10,25,50,100]
Bernier =[1.1002,1.1229,1.1326,1.1416,1.1469,1.1513]
Zhang = [1.2967,1.3696,1.4005,1.4295,1.4464,1.4605]
Muise = [1.69036,1.80519,1.88121,1.97727,2.04853,2.11927]
Beck = [0.77,0.94,1.06,1.23,1.33,1.37]
Beck_calculated = [1.47,1.62,1.68,1.73,1.76,1.78]
Beck_surge = [0.75,0.929,1.038,1.17,1.26,1.34]

#plt.plot(x,Bernier,'-o',ms=10,label='Bernier et al., 2006')
#plt.plot(x,Zhang,'-o',ms=10,label='Zhang et al., 2019')
#plt.plot(x,Beck_calculated,'-o',ms=10,label='Total water level: station 2330')
plt.plot(x,Beck,'-o',ms=10,label='Beck et al., 2019')
plt.plot(x,Beck_surge,'-o',ms=10,label='Surge: station 2330')
plt.xticks(x)


plt.xlabel('Return periods')
plt.ylabel('Water level (m)')
plt.title('Rivière au Renard: lat =  48.99777  lon =-64.3931')
plt.grid('on')
plt.legend(framealpha=1, frameon=True,loc='lower right');
plt.savefig('riviere_au_Renard3.png')



#Riviere du Goffre
x= [2,5,10,25,50,100]
Bernier =[2.9839,3.09299106272931,3.13938768082586,3.18278560134252,3.20805000699372,3.22918221943906]
Zhang = [3.2926,3.3837,3.4224,3.4587,3.4789,3.4974]
Muise = [2.02753,2.11503,2.17296,2.24616,2.30046,2.35437]
Beck = [1.28,1.59,1.82,2.14,2.4,2.67]
Beck_calculated = [3.648,3.854,3.979,4.126,4.227,4.321]
Beck_surge = [1.26,1.65,1.9,2.2,2.42,2.63]

#plt.plot(x,Bernier,'-o',ms=10,label='Bernier et al., 2006')
#plt.plot(x,Zhang,'-o',ms=10,label='Zhang et al., 2019')
#plt.plot(x,Beck_calculated,'-o',ms=10,label='Total water level: station 3057')
plt.plot(x,Beck,'-o',ms=10,label='Beck et al., 2019')
plt.plot(x,Beck_surge,'-o',ms=10,label='SURGE: station 3057')
plt.xticks(x)


plt.xlabel('Return periods')
plt.ylabel('Water level (m)')
plt.title('Rivière du Gouffre: lat =  47.449  lon =-70.3655')
plt.grid('on')
plt.legend(framealpha=1, frameon=True,loc='lower right');
plt.savefig('riviere_du_Goffre_4.png')
#########################################################33
data = {'Station':['Saguenay', 'Cape Breton', 'Gaspesie', 'Cote Nord'], 
        'lat':[-69.72,-60.68,-64.164,-64.94],
        'lon':[48.13,46.942,48.577,50.24]} 
...
Zhang['type']='Zhang et al., 2013'

merge_Zhang_DEH = pd.concat([Zhang,DEH])
...
x= [2,5,10,20,25,40, 50,100]
Bernier_calculated =[3.0616,3.1529,3.1917,3.2202,3.2279,3.2427,3.2491,3.2667]
Bernier_sim = [np.nan,np.nan,np.nan,np.nan,np.nan,3.2647,np.nan,3.3146]

plt.plot(x,Bernier_sim,'-o',ms=10,label='Bernier et al., 2006')
plt.plot(x,Bernier_calculated,'-o',ms=10,label='Bernier_calculated')
plt.xticks(x)
...
plt.grid('on')
plt.legend(framealpha=1, frameon=True,loc = 'lower right');
plt.savefig('Saguenay_comp.png')

#comparison of three DB at extreme water level value prediction of cape_Breton
x= [2,5,10,20,25,40, 50,100]
Bernier_calculated =[0.9659,1.0991,1.1557,1.1973,1.2086,1.2302,1.2394,1.2652]
Bernier_sim = [np.nan,np.nan,np.nan,np.nan,np.nan,1.0850,np.nan,1.1585]
...
plt.grid('on')
#plt.legend(framealpha=1, frameon=True,loc = 'lower left');
plt.savefig('Cape_Breton_comp.png')


#Gaspesie
x= [2,5,10,20,25,40, 50,100]
Bernier_calculated =[1.2638,1.3591,1.3996,1.4293,1.4374,1.4528,1.4595,1.4779]
Bernier_sim = [np.nan,np.nan,np.nan,np.nan,np.nan,1.3945,np.nan,1.4685]
...
#Cote-Nord
x= [2,5,10,20,25,40, 50,100]
Bernier_calculated =[1.8612,1.9471,1.9836,2.0104,2.0177,2.0316,2.0376,2.0542]
Bernier_sim = [np.nan,np.nan,np.nan,np.nan,np.nan,2.0024,np.nan,2.0798]
